...
class TopTechnology extends StatefulWidget {
  const TopTechnology({Key? key}) : super(key: key);

  @override
  State<TopTechnology> createState() => _TopTechnologyState();
}

class _TopTechnologyState extends State<TopTechnology> {
  @override
  Widget build(BuildContext context) {
    var size = MediaQuery.of(context).size;
    return Scaffold(
      backgroundColor: blacktheme,
      body: FutureBuilder(future: getGenreNews("technology"), builder: (context,AsyncSnapshot snapshot){
        if(snapshot.hasData) {
